import type {WikibaseEntity} from './wdtypes';
...
export type MwApiServiceServiceInit = {
	server: string;
	apiEndpoint?: string;
	fetch?: FetchFn;
};
...
export class MwApiService {
	public static getInstance(init: MwApiServiceServiceInit): MwApiService {
	...
	async * callGen(query: WdQueryParameters) {
		...
		if (queryField) {
			...
			const promises: Array<Promise<Array<Record<string, any>>>> = [];
